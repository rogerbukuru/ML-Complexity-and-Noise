...
neural_net = function(X, Y, theta,m,nu) {
   ...
   y_hat         = t(A2)
   error         = rep(0,N)
   y_i_1         = which(Y==c(1,0,0)) # class 1
   y_i_2         = which(Y==c(0,1,0)) # class 2
   y_i_3         = which(Y==c(0,0,1)) # class 3
   error[y_i_1]  = y_i_1%*%log(y_hat[y_i_1])
   error[y_i_2]  = y_i_2%*%log(y_hat[y_i_2])
   error[y_i_3]  = y_i_3%*%log(y_hat[y_i_3])
