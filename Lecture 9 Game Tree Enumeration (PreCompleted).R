...
draw_game_tree=function(m,k,x0=0,d = 0,lims=c(-1,1))
{
   ...
   {
   	  g = c(g,game_state$winner)
   	  #points(x0,d,pch = 16, col = c('blue','green','red')[game_state$winner+2])
   	  text(x0,d,labels = c('X','-','O')[game_state$winner+2],cex = 0.5)
   	  return(g)
   }else{
   	Index = which(m == 0)
   	...
   	for(i in 1:length(Index))
   	{
   		segments(x0,d,x_mids[i],d+1,col = 'grey')
   		x = m
   		x[Index[i]] = k
